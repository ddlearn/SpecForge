...
| DFlash / Domino / D-PACE | `training.num_anchors` (`512`), `training.loss_decay_gamma` (`null`), `training.objective_chunk_blocks` (`128`; `0` materializes all objective logits), `training.loss_type` (`dflash`), `training.dpace_alpha` (`0.5`), `training.loss_denominator` (DFlash/D-PACE only; `null` preserves the objective default: `loss_weight_sum` for DFlash and `batch_size` for D-PACE; explicit choices are `valid_token_count`, `loss_weight_sum`, and D-PACE-only `batch_size`), `training.lambda_base_start` (`1.0`), `training.lambda_base_decay_ratio` (`0.5`) |
